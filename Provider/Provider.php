...
abstract class Provider extends CComponent
{
    ...
    // public $storageClass = 'OAuth2Yii\Storage\SessionClientStorage';
	 public $storageClass = 'OAuth2Yii\Component\SessionClientStorage';
    ...
    /**
     * @param string|null|boolean $id of the user or null to use the current user id (default).
     *  If `true` the access token for grant type `client_credentials` is returned.
    ...
    {	echo "------->".$id;
        $type = $id===true ? AccessToken::TYPE_CLIENT : AccessToken::TYPE_USER;		dump($id==true); echo "<br>type-->".$type;
        if($id===null) {
            $id = Yii::app()->user->id;
            if($id===null) {
                return null;
        ...
        $token = $this->getStorage()->loadToken($id, $type, $this->name);			//dump($this->getStorage());
    ...
    public function sendGuzzleRequest($request,$id = null)
    {
        ...
        if($token===null) {
            YII_DEBUG && Yii::trace("Could not send Guzzle request: No token available",'oauth2.provider.provider');
            return false;
        }

        YII_DEBUG && Yii::trace("Sending Guzzle request to {$request->getUrl()} with access token '$token'",'oauth2.provider.provider');
